    ...
    private static List<NamedTest> generateTimeGuardianTests() {
        ...
        tests.add(new NamedTest("boss_time_guardian_phase_transition", helper -> {
            ...
            helper.runAfterDelay(5, () -> {
                ...
                if (guardian.getPhase() != TimeGuardianEntity.PHASE_1) {
                    helper.fail(Component.literal("Time Guardian should start in Phase 1, was Phase " + guardian.getPhase()));
                    return;
                }
                ...
                helper.runAfterDelay(2, () -> {
                    if (guardian.getPhase() == TimeGuardianEntity.PHASE_2) {
                        ...
                        helper.fail(Component.literal("Time Guardian should be in Phase 2 at " +
                ...
                helper.runAfterDelay(10, () -> {
                    ...
                    if (mockPlayer.getHealth() < mockPlayer.getMaxHealth()) {
                        ...
                        if (guardian.getPhase() == TimeGuardianEntity.PHASE_2) {
                            ...
                            helper.fail(Component.literal("Time Guardian AoE: Phase was " + guardian.getPhase() +
                                ", expected Phase 2"));
                        }
                    }
                });
        ...
        tests.add(new NamedTest("boss_time_guardian_post_teleport_delay", helper -> {
            ...
            helper.runAfterDelay(5, () -> {
                ...
                if (guardian.isInPostTeleportDelay()) {
                    helper.fail(Component.literal("Time Guardian should not start in post-teleport delay"));
                    return;
                }
                ...
                helper.runAfterDelay(2, () -> {
                    ...
                    if (guardian.getPhase() == TimeGuardianEntity.PHASE_2) {
                        ...
                        helper.fail(Component.literal("Time Guardian phase transition failed, was Phase " + guardian.getPhase()));
                    }
                });
            });
        }, BOSS_TIMEOUT));
        ...
        tests.add(new NamedTest("boss_time_guardian_loot_items_exist", helper -> {
            helper.runAfterDelay(1, () -> {
                ...
                if (key.isEmpty()) {
                    helper.fail(Component.literal("KEY_TO_MASTER_CLOCK item could not be created"));
                } else if (enhancedClockstone.isEmpty()) {
                    helper.fail(Component.literal("ENHANCED_CLOCKSTONE item could not be created"));
                } else {
                    helper.succeed();
                }
            });
    ...
    private static List<NamedTest> generateTimeTyrantTests() {
        ...
        tests.add(new NamedTest("boss_time_tyrant_phase_transitions", helper -> {
            ...
            helper.runAfterDelay(5, () -> {
                ...
                if (tyrant.getPhase() != TimeTyrantEntity.PHASE_1) {
                    helper.fail(Component.literal("Time Tyrant should start in Phase 1, was Phase " + tyrant.getPhase()));
                    return;
                }
                ...
                helper.runAfterDelay(2, () -> {
                    if (tyrant.getPhase() != TimeTyrantEntity.PHASE_2) {
                        helper.fail(Component.literal("Time Tyrant should be Phase 2 at 50% HP, was Phase " + tyrant.getPhase()));
                        return;
                    }
                    ...
                    helper.runAfterDelay(2, () -> {
                        if (tyrant.getPhase() == TimeTyrantEntity.PHASE_3) {
                            ...
                            helper.fail(Component.literal("Time Tyrant should be Phase 3 at 20% HP, was Phase " + tyrant.getPhase()));
                        }
                    });
                });
            });
                ...
                helper.runAfterDelay(5, () -> {
                    ...
                    if (currentHp > triggerHp) {
                        ...
                        if (tyrant.getPhase() == TimeTyrantEntity.PHASE_3) {
                            ...
                            helper.fail(Component.literal("Time Tyrant at 15% HP should be Phase 3, was Phase " +
        ...
        tests.add(new NamedTest("boss_time_tyrant_loot_items_exist", helper -> {
            helper.runAfterDelay(1, () -> {
                ...
                if (fragment.isEmpty()) {
                    helper.fail(Component.literal("FRAGMENT_OF_STASIS_CORE item could not be created"));
                } else if (eye.isEmpty()) {
                    helper.fail(Component.literal("EYE_OF_CHRONOS item could not be created"));
