...
import com.chronodawn.client.renderer.mobs.TimeboundRabbitRenderState;
 ...
 * Adapted for 1.21.2 RenderState pattern with rabbit-style animations.
 */
public class TimeboundRabbitModel extends EntityModel<TimeboundRabbitRenderState> {
    ...
    @Override
    public void setupAnim(TimeboundRabbitRenderState state) {
        super.setupAnim(state);

        float ageInTicks = state.ageInTicks;
        float headYaw = state.yRot;
        float headPitch = state.xRot;
        float jumpRotation = state.jumpRotation;
        ...
        float jumpAmount = Mth.sin(jumpRotation * (float) Math.PI);
