...
/**
 * Timeline Strider - Enderman-like hostile mob with teleportation ability.
 *
 * Neutral until stared at by a player, then becomes hostile.
 * Teleports randomly when damaged.
 *
 * Attributes:
 * - Health: 40 (20 hearts)
 ...
 *
 * Drops:
 * - Time Crystal (0-1, with looting bonus)
 */
public class TimelineStriderEntity extends Monster {

    private int targetChangeTime;

    public TimelineStriderEntity(EntityType<? extends TimelineStriderEntity> entityType, Level level) {
    ...
    private boolean isLookingAtMe(Player player) {
        ...
        // Player is looking at this entity if dot product is high enough
        // and within follow range
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide) {
            // Check for player stare to set target
    ...
    public boolean hurtServer(ServerLevel serverLevel, DamageSource source, float amount) {
        ...
        if (result && this.isAlive()) {
            // Teleport randomly when damaged
            teleportRandomly();
        }
        return result;
    }
    ...
    private boolean tryTeleportTo(double x, double y, double z) {
        ...
        // Move up to stand on top of the block
        pos.move(0, 1, 0);

        // Check if there's enough space (2 blocks of air above)
     ...
     */
    public static boolean checkTimelineStriderSpawnRules(
        EntityType<TimelineStriderEntity> entityType,
