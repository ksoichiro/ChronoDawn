...
/**
 * Main mod class for NeoForge (1.21.5 version).
 *
 * This version includes GameTest registration using the new 1.21.5 registry-based system.
 */
...
public class ChronoDawnNeoForge {
    ...
    public ChronoDawnNeoForge(IEventBus modEventBus) {
        ...
        ChronoDawnGameTestsNeoForge.register(modEventBus);
        ...
        ChronoDawn.LOGGER.info("ChronoDawn Mod (NeoForge 1.21.5) initialized");
    }
    ...
    private void onBlockBreak(BreakBlockEvent event) {
        ...
        if (event.getLevel() instanceof net.minecraft.world.level.Level level) {
            if (BlockProtectionHandler.isProtected(level, event.getPos())) {
                ...
                // 26.1.2: Player#displayClientMessage(Component, boolean) removed;
                // action-bar case is now sendOverlayMessage(Component)
