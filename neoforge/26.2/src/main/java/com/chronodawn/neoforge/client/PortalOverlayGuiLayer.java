 ...
 * separate stage this mod can't hook via a plain Mixin injection. NeoForge's own
 * {@link GuiLayer} still receives a drawing-capable {@link GuiGraphicsExtractor} and is
 * composited on top of the vanilla HUD by {@code RegisterGuiLayersEvent}, so it replaces
 * the {@code GuiPortalOverlayMixin} approach used by every other supported version.
 */
public class PortalOverlayGuiLayer implements GuiLayer {
    ...
    private void chronodawn$renderPortalEffects(GuiGraphicsExtractor guiGraphics) {
        ...
        float fadeAlpha = com.chronodawn.client.PortalFadeHandler.getFadeAlpha();
        if (fadeAlpha > 0.0f) {
