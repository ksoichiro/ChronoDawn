...
import com.chronodawn.client.renderer.mobs.ParadoxCrawlerRenderState;
 ...
 */
public class ParadoxCrawlerModel extends EntityModel<ParadoxCrawlerRenderState> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        // Right legs (extend to the left in model space, negative X)
        ...
        // Left legs (extend to the right in model space, positive X)
    ...
    @Override
    public void setupAnim(ParadoxCrawlerRenderState state) {
        ...
        // Spider-like leg animation (alternating pairs)
        float legSpread = 0.7853982F; // pi/4

        // Right legs swing with alternating pattern
        ...
        // Leg walking animation (Y rotation for forward/backward swing)
        float legSwing1 = -(Mth.cos(walkPos * 0.6662F * 2.0F) * 0.4F) * walkSpeed;
        float legSwing2 = -(Mth.cos(walkPos * 0.6662F * 2.0F + (float) Math.PI) * 0.4F) * walkSpeed;
        float legSwing3 = -(Mth.cos(walkPos * 0.6662F * 2.0F + ((float) Math.PI / 2F)) * 0.4F) * walkSpeed;
        float legSwing4 = -(Mth.cos(walkPos * 0.6662F * 2.0F + ((float) Math.PI * 3F / 2F)) * 0.4F) * walkSpeed;
