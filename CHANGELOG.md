...
- **Repair ingredient `c:` tags** — equipment repair now accepts `c:gems/time_crystal`, `c:gems/entropy_crystal` and `c:dusts/temporal_amber` on Minecraft 1.21.1+, so compatible pack-provided materials work without Java replacements. **Minecraft 1.21.2 is excluded** on both Fabric and NeoForge and keeps the original vanilla repair tags instead — using a custom-namespace tag as a `ToolMaterial`/`ArmorMaterial` repair ingredient crashes NeoForge 1.21.2 at startup with an unbound-tag error, a known upstream NeoForge bug fixed only from `21.3.7-beta` onward (MC 1.21.3+), with no fixed release ever published for MC 1.21.2 itself. See [docs/modpack-integration.md](docs/modpack-integration.md#equipment-repair-materials).
...
- **Portal lookups could match a different dimension's portal.** `PortalRegistry` tracked portal positions across all dimensions in a single map, so a Chrono Dawn portal frame could collide with an Overworld portal at the same `BlockPos` (a real risk given the mod's 1:1 coordinate mapping). When that happened, the Chrono Dawn side never registered or activated, and no `PortalOpenedEvents` reignition callback fired. Portal position lookups are now scoped per dimension.
...
- **Forgotten Library and Desert Clock Tower spawn biomes expanded** — Forgotten Library now generates in Plains, Forest, Prairies, and Faded Plains (was Plains only), so the Portal Stabilizer blueprint is no longer locked behind a single biome; Desert Clock Tower now also generates in Faded Plains (only Ocean remains excluded). Existing worlds keep already-generated structures; only newly explored chunks reflect the change.
...
- **Enchantments for ChronoDawn tools, armor, and shields** on 1.21.1+ — every Clockstone / Enhanced Clockstone / Entropy Crystal / Temporal Amber / Chronoblade / Time Tyrant's Mail / Echoing Time Boots piece, plus all three custom shields, can now receive enchantments at the enchanting table and via anvil + enchanted book. They were silently rejected on 1.21.1+ because Minecraft 1.21 moved enchantment eligibility to the `#minecraft:enchantable/*` item tags and the custom equipment had never been added to them; 1.20.1 was unaffected since its check is based on Java class inheritance.
- **ChronoDawn portal**:
...
- **Time Hourglass recipe** now accepts both `minecraft:sand` and `chronodawn:temporal_sand` via the new `#chronodawn:sand` item tag, so the recipe can be crafted inside the Chrono Dawn dimension where worldgen places only `temporal_sand`.
...
- **Time Hourglass Chronicle entry** — the "Required Materials" page now matches the actual recipe (4 Glass + 2 Clockstone + 1 Sand / Temporal Sand). The previous wording listed a Glass Bottle, 4 Clockstone, 2 Time Crystals, and 2 Redstone Dust, none of which match the shipped recipe. The "Crafting Tips" page also clarifies that Sand and Temporal Sand are interchangeable. Both `en_us` and `ja_jp` updated.
...
- **Chronicle in-game entries reconciled with the actual implementation** — corrected numerous descriptions that had drifted from the shipped content: Clockstone Ore is mined directly for 3-4 Clockstone (no furnace smelting); Clockstone / Enhanced Clockstone equipment is crafted without Time Crystals; the Enhanced Clockstone armor full set now lists its real protection stats (the previously advertised Time-Distortion / Slowness immunity never applied to players); Clockwork Colossus abilities corrected (Overcharge grants Speed II, Repair Protocol is a one-time ~40% HP heal); the unimplemented Temporal Phantom clones description removed; the Ultimate Artifacts page now lists the real items (Chronoblade, Time Tyrant's Mail, Echoing Time Boots, Unstable Pocket Watch); non-existent Ancient Ruins inscriptions / "Ancient Knowledge" removed; and Japanese item & boss names plus the English "Key to Master Clock" aligned with the actual in-game names. Both `en_us` and `ja_jp` updated.
...
[Unreleased]: https://github.com/ksoichiro/ChronoDawn/compare/v0.8.0...HEAD
