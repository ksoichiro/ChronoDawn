...
| **Fix `structure_contains_9_smooth_stone_old_sundial`** (confirmed failing on `main` @`9cef5105` on 2026-07-25): `./gradlew :fabric:runGameTest -Ptarget_mc_version=1.20.1` reports `BUILD FAILED` on a clean checkout, so `checkAll` cannot pass end-to-end today. A full `checkAll` run fails this one test — and only this one — on 1.20.1, 1.21.1, 1.21.2, 1.21.3 and 1.21.4, on both loaders; it passes on 1.21.5+. | Unrelated to any in-flight feature work; blocks the "checkAll is green" invariant every plan's done-criteria assumes. Version split suggests an era-specific structure or block-mapping issue rather than a test bug. | S |
