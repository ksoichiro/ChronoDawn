...
| **Fix the Fabric 1.21.5+ GameTest blind spot** (found 2026-07-25): `fabric/<v>/src/gametest/java/com/chronodawn/gametest/ChronoDawnGameTests.java` for 1.21.5–1.21.11 overrides `fabric/base`'s same-named class and drops its `RegistryDrivenTestGenerator.generateAllTests()` call (`fabric/base/src/gametest/…/ChronoDawnGameTests.java:29`). Every registry-driven GameTest — boss fights, registry consistency, translations, advancements, structures, portals, mob behavior — silently does not run on Fabric for 7 of 11 versions. Only 9 hardcoded tests execute. NeoForge is unaffected: it wires `generateAllTests()` in every version. | Expect a wave of genuine failures the first time the suppressed tests actually run — budget for triage, not just the wiring change. This is the same silent-coverage-loss class the row above exists to prevent. | M |
| **Fix `structure_contains_9_smooth_stone_old_sundial`** (confirmed failing on `main` @`9cef5105` on 2026-07-25): `./gradlew :fabric:runGameTest -Ptarget_mc_version=1.20.1` currently reports `BUILD FAILED` on a clean checkout, so `checkAll` cannot pass end-to-end today. | Unrelated to any in-flight feature work; blocks the "checkAll is green" invariant every plan's done-criteria assumes. | S |
