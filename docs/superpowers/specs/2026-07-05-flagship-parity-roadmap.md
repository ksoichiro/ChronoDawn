...
| ~~**Fix the Fabric 1.21.5+ GameTest blind spot**~~ — **resolved 2026-07-28. The diagnosis below was wrong.** It is not a shadowing mistake that can be undone by restoring a call: 1.21.5 rewrote GameTest to be registry-based and Fabric API dropped `FabricGameTest` and `@GameTestGenerator` with it, leaving only a `@GameTest` annotation that cannot express runtime-generated tests. The per-version override existed because it had to. NeoForge has `RegisterGameTestsEvent`; Fabric exposes no equivalent and does it internally via a `RegistryDataLoader` mixin. Fixed by `fabric/base-1.21.5+/src/gametest/java/.../ChronoDawnFabricTestRegistrar.java`, a `ModInitializer` that registers each generated test body in `BuiltInRegistries.TEST_FUNCTION` and then adds a `FunctionGameTestInstance` per test to the `test_instance` dynamic registry from `DynamicRegistrySetupCallback` (`fabric-registry-sync-v0`) — the same phase Fabric's own mixin uses, so no mixin of ours is needed. Fabric 1.21.5, 1.21.6 and 1.21.11 all now report **2292 required tests passed**, identical to NeoForge; previously Fabric ran 10. | The nine `@GameTest` methods (player input + Faded Plains) stay annotation-driven and are excluded from `generateAllTests()` on 1.21.5+, so there is no duplicate-id collision. 1.21.11 renamed `ResourceKey.location()` along with `ResourceLocation`, so the registrar carries the sanitized id path as a `String` and rebuilds ids through `CompatResourceLocation` rather than reading them back off the key. | M |
| ~~(original diagnosis, kept for context)~~ **Fix the Fabric 1.21.5+ GameTest blind spot** (found 2026-07-25): `fabric/<v>/src/gametest/java/com/chronodawn/gametest/ChronoDawnGameTests.java` for 1.21.5–1.21.11 overrides `fabric/base`'s same-named class and drops its `RegistryDrivenTestGenerator.generateAllTests()` call (`fabric/base/src/gametest/…/ChronoDawnGameTests.java:29`). Every registry-driven GameTest — boss fights, registry consistency, translations, advancements, structures, portals, mob behavior — silently does not run on Fabric for 7 of 11 versions. NeoForge is unaffected: it wires `generateAllTests()` in every version. The gap is stark in one `gameTestAll` run on 1.21.11: NeoForge reported "All 1090 required tests passed", Fabric reported "All 10 required tests passed". (NeoForge is now at 2292 after the row below; Fabric is unchanged, so the gap has widened.) | Expect a wave of genuine failures the first time the suppressed tests actually run — budget for triage, not just the wiring change. This is the same silent-coverage-loss class the row above exists to prevent. **Update 2026-07-28:** the sibling row below un-stubbed the same 7 suites for NeoForge and produced zero failures, so the "wave of failures" assumption is now evidence-free. | M |
| ~~**Un-stub the shared GameTest suites on 1.21.5–1.21.11**~~ — **resolved 2026-07-28.** `common/<ver>/build.gradle` had commented out `srcDir '../gametest/src/main/java'` because `GameTestHelper.fail(String)` became `fail(Component)` in 1.21.5, leaving 7 of the 8 shared suites (`AdvancementTests`, `BlockProtectionTests`, `MobBehaviorTests`, `PortalTests`, `RegistryConsistencyTests`, `StructureTests`, `TranslationKeyTests`) as empty per-version stubs on 7 of 11 versions — both loaders. Fixed by adding `CompatGameTestHelper.fail(GameTestHelper, String)` to all 11 versions (passthrough pre-1.21.5, `Component.literal` from 1.21.5), rewriting the 61 `helper.fail(` call sites in the shared classes, re-enabling the `srcDir` with `FadedPlainsTests.java` excluded (it stays per-version — registered via `@GameTest` / `Consumer` constants from 1.21.5), and deleting the 49 stub files. NeoForge 1.21.11 went **1090 → 2292 required tests, zero failures**; `checkAll` is green. | Only one genuine API drift beyond `fail()` surfaced: 1.21.11's `ResourceLocation` → `Identifier` rename broke 16 local declarations in 3 shared classes, fixed with `var` so one source serves all 11 versions. The per-version `buildAll` wrapper hid the compiler output entirely — the error was only visible by running `./gradlew clean build -Ptarget_mc_version=1.21.11 -x test` directly. | M |
