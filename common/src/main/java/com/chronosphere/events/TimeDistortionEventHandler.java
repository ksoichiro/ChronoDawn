...
public class TimeDistortionEventHandler {
    ...
    private static void onServerLevelTick(ServerLevel level) {
        ...
        if (!timeSpeedMap.containsKey(dimensionKey)) {
            ...
            Chronosphere.LOGGER.info("TimeDistortionEventHandler: Initialized for Chronosphere with speed 1.0x");
        }
        ...
        if (targetTime != null) {
            ...
            if (remainingTicks > 0) {
                ...
                Chronosphere.LOGGER.info("TimeDistortionEventHandler: Sleep skip advancing... {} / {} remaining",
                    remainingTicks - ticksThisTick, remainingTicks);
            } else {
                // Target reached!
                sleepSkipTargetTimeMap.remove(dimensionKey);
                Chronosphere.LOGGER.info("TimeDistortionEventHandler: Sleep skip complete! Final time: {}", currentTime);
            }
        ...
        if (timeUntilChange <= 0) {
            ...
            Chronosphere.LOGGER.info("TimeDistortionEventHandler: Speed changed from {}x to {}x (duration: {} ticks)",
    ...
    public static void requestSleepSkip(ServerLevel level, long targetTime) {
        ...
        Chronosphere.LOGGER.info("Sleep skip requested: target time = {}", targetTime);
    }
}
