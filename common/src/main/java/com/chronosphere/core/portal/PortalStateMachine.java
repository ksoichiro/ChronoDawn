 ...
 *
 * This class handles all state transitions for Chronosphere portals and enforces
...
public class PortalStateMachine {
    ...
    public boolean activate() {
        if (currentState != PortalState.INACTIVE) {
            Chronosphere.LOGGER.warn("Cannot activate portal {} - current state is {}", portalId, currentState);
            return false;
        }

        currentState = PortalState.ACTIVATED;
        Chronosphere.LOGGER.info("Portal {} activated at {} in dimension {}",
    ...
    public boolean deactivate() {
        if (currentState != PortalState.ACTIVATED) {
            Chronosphere.LOGGER.warn("Cannot deactivate portal {} - current state is {}", portalId, currentState);
            return false;
        }

        currentState = PortalState.DEACTIVATED;
        Chronosphere.LOGGER.info("Portal {} deactivated at {} in dimension {}",
            portalId, position, sourceDimension.location());
        return true;
    }
    ...
    public boolean stabilize() {
        if (!currentState.canBeStabilized()) {
            Chronosphere.LOGGER.warn("Cannot stabilize portal {} - current state is {}", portalId, currentState);
            return false;
        }

        currentState = PortalState.STABILIZED;
        Chronosphere.LOGGER.info("Portal {} stabilized at {} in dimension {}",
            portalId, position, sourceDimension.location());
        return true;
    }
