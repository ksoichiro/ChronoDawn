...
public class PhantomCatacombsBossRoomPlacer {
    ...
    private static List<BlockPos> findDeadEndMarkers(ServerLevel level, BlockPos structureOrigin, net.minecraft.world.level.levelgen.structure.BoundingBox boundingBox) {
        ...
        if (!markers.isEmpty()) {
            Chronosphere.LOGGER.info("Found {} Crying Obsidian markers at positions:", markers.size());
            for (int i = 0; i < Math.min(markers.size(), 5); i++) {
                BlockPos marker = markers.get(i);
                Chronosphere.LOGGER.info("  Marker {}: {} (distance from origin: {})",
                    i + 1,
                    marker,
            ...
            if (markers.size() > 5) {
                Chronosphere.LOGGER.info("  ... and {} more", markers.size() - 5);
            }
        } else {
    ...
    private static BlockPos placeRoom7(ServerLevel level, BlockPos deadEndPos) {
        ...
        if (templateOptional.isEmpty()) {
            Chronosphere.LOGGER.error("Failed to load room_7 template: {}", ROOM_7_TEMPLATE);
            return null;
        }
        ...
        if (waterRemoved > 0) {
            Chronosphere.LOGGER.info("Removed {} water blocks before room_7 placement", waterRemoved);
        }
        ...
        Chronosphere.LOGGER.info("Removed Crying Obsidian marker at {}", deadEndPos);
        ...
        if (waterloggedRemoved > 0) {
            Chronosphere.LOGGER.info("Removed {} waterlogged blocks after room_7 placement", waterloggedRemoved);
        }
    ...
    private static Direction detectExitDirection(ServerLevel level, BlockPos connectorPos) {
        ...
        Chronosphere.LOGGER.info("Detecting exit direction from connector at {}", connectorPos);
        ...
        for (Direction dir : new Direction[]{Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST}) {
            ...
            Chronosphere.LOGGER.info("  Direction {}: {} air/non-solid blocks", dir, airBlocks);
    ...
    private static boolean placeBossRoom(ServerLevel level, BlockPos connectorPos) {
        ...
        if (waterRemoved > 0) {
            Chronosphere.LOGGER.info("Removed {} water blocks before boss_room placement", waterRemoved);
        }
        ...
        try {
            ...
            if (processorList != null) {
                Chronosphere.LOGGER.info("Found processor list {}: {} processors", processorListId, processorList.list().size());
                for (var processor : processorList.list()) {
                    settings.addProcessor(processor);
                    Chronosphere.LOGGER.info("  Added processor: {}", processor.getClass().getSimpleName());
                }
                Chronosphere.LOGGER.info("Successfully applied {} structure processors to boss_room placement", processorList.list().size());
            } else {
                Chronosphere.LOGGER.error("Processor list {} not found in registry! Boss_room WILL have waterlogging issues", processorListId);
                Chronosphere.LOGGER.error("Available processor lists: {}", processorListRegistry.keySet());
            }
        } catch (Exception e) {
            Chronosphere.LOGGER.error("Failed to load processor list {}: {}", processorListId, e.getMessage(), e);
        }
        ...
        if (waterloggedRemoved > 0) {
            Chronosphere.LOGGER.info("Removed {} waterlogged blocks after boss_room placement", waterloggedRemoved);
        }
            ...
            if (structureLocation != null && structureLocation.equals(PHANTOM_CATACOMBS_ID)) {
                ...
                for (long chunkLong : chunks) {
                    ...
                    if (startForStructure != null && startForStructure.isValid()) {
                        ...
                        if (box != null) {
                            Chronosphere.LOGGER.info("Successfully retrieved bounding box from StructureStart: {}", box);
                            return box;
                        }
                    }
                }
                ...
                Chronosphere.LOGGER.warn("Could not get bounding box from StructureStart, scanning chunks for structure blocks...");
                ...
                if (foundAny) {
                    ...
                    Chronosphere.LOGGER.info("Created bounding box from chunk scan: {}", scannedBox);
                    return scannedBox;
                }
            }
        }
    ...
    public static void processStructure(ServerLevel level, ChunkPos chunkPos) {
        ...
        if (room7TemplateOpt.isEmpty() || bossRoomTemplateOpt.isEmpty()) {
            Chronosphere.LOGGER.error("Failed to load templates for boss_room placement");
        ...
        if (candidates.isEmpty()) {
            Chronosphere.LOGGER.error("No placement candidates generated (template error?)");
            dimensionProcessed.add(structureOrigin);
            return;
        }
        ...
        if (!collisionFree.isEmpty()) {
            ...
            if (!minimalCollision.isEmpty()) {
                ...
                Chronosphere.LOGGER.warn("Colliding room origins: {}", selected.collidingRoomOrigins);
            } else {
    ...
    private static List<PlacementCandidate> evaluateAllPlacements(
        ...
        if (room7Marker == null || room7Connector == null || bossRoomConnector == null) {
            Chronosphere.LOGGER.error("Cannot evaluate placements: missing markers in templates");
            return candidates;
        }
    ...
    private static boolean placeBossRoomIndependently(
        ...
        if (templateOptional.isEmpty()) {
            Chronosphere.LOGGER.error("Boss_room template {} not found", BOSS_ROOM_TEMPLATE);
            return false;
        }
        ...
        try {
            ...
            if (processorList != null) {
                Chronosphere.LOGGER.info("Found processor list {}: {} processors (hidden chamber)", processorListId, processorList.list().size());
                for (var processor : processorList.list()) {
                    settings.addProcessor(processor);
                }
                Chronosphere.LOGGER.info("Successfully applied {} structure processors to hidden chamber placement", processorList.list().size());
            } else {
                Chronosphere.LOGGER.error("Processor list {} not found for hidden chamber", processorListId);
            }
        } catch (Exception e) {
            Chronosphere.LOGGER.error("Failed to load processor list for hidden chamber: {}", e.getMessage(), e);
        }
    ...
    public static void register() {
        ...
        Chronosphere.LOGGER.info("Registered PhantomCatacombsBossRoomPlacer");
    }
}
