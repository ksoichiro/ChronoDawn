 ...
 *
 * Spawn Conditions:
 * - Location: Guardian Vault structure (Chronosphere dimension)
...
public class ChronosWardenSpawner {
    private static final ResourceLocation GUARDIAN_VAULT_ID = ResourceLocation.fromNamespaceAndPath(
        Chronosphere.MOD_ID,
        "guardian_vault"
    );
    ...
    public static void register() {
        ...
        LifecycleEvent.SERVER_LEVEL_LOAD.register(level -> {
            if (level instanceof ServerLevel serverLevel) {
                Chronosphere.LOGGER.info("Chronos Warden Spawner initialized for dimension: {}", serverLevel.dimension().location());
            }
        });

        Chronosphere.LOGGER.info("Registered ChronosWardenSpawner");
    }
        ...
        for (int x = -10; x <= 10; x++) {
            for (int z = -10; z <= 10; z++) {
                ...
                if (hasGuardianVault(level, chunkPos)) {
                    ...
                    Chronosphere.LOGGER.info("Found Guardian Vault structure at chunk {} (block pos: {})", chunkPos, structurePos);
    ...
    private static void spawnChronosWardenAtBossRoom(ServerLevel level, ChunkPos chunkPos) {
        ...
        for (var entry : structureManager.getAllStructuresAt(chunkBlockPos).entrySet()) {
            ...
            if (structureLocation != null && structureLocation.equals(GUARDIAN_VAULT_ID)) {
                Chronosphere.LOGGER.info("Attempting to spawn Chronos Warden at Guardian Vault in chunk {}", chunkPos);
                ...
                if (bossRoomSpawnPos == null) {
                    Chronosphere.LOGGER.warn("Could not find Guardian Vault boss room at chunk {}", chunkPos);
                    return;
                }
                ...
                if (isWardenNearby(level, bossRoomSpawnPos)) {
                    Chronosphere.LOGGER.debug("Chronos Warden already exists near {}", bossRoomSpawnPos);
                    return;
                }
    ...
    private static BlockPos findBossRoom(ServerLevel level, BlockPos searchCenter) {
        ...
        Chronosphere.LOGGER.info("Searching for Guardian Vault Door from Y={} to Y={} around {}", maxY, minY, searchCenter);
                    ...
                    if (blockState.getBlock() instanceof com.chronosphere.blocks.BossRoomDoorBlock) {
                        ...
                        if (blockEntity instanceof com.chronosphere.blocks.BossRoomDoorBlockEntity doorEntity) {
                            if (doorEntity.isGuardianVaultDoor()) {
                                ...
                                Chronosphere.LOGGER.info("Found Guardian Vault Door at {} facing {}, spawn at {}",
        ...
        Chronosphere.LOGGER.warn("Could not find Guardian Vault Door near {}", searchCenter);
        return null;
    }
    ...
    public static void spawnOnDoorOpen(ServerLevel level, BlockPos doorPos, BlockState doorState) {
        Chronosphere.LOGGER.info("Guardian Vault Door opened at {}", doorPos);

        // Check if we've already spawned from this door
        if (spawnedDoors.contains(doorPos)) {
            Chronosphere.LOGGER.info("Chronos Warden already spawned from this door - not spawning again");
            return;
        }
        ...
        if (isWardenNearby(level, spawnPos)) {
            Chronosphere.LOGGER.info("Chronos Warden already exists near {} - not spawning", spawnPos);
            return;
        }
        ...
        if (warden != null) {
            ...
            Chronosphere.LOGGER.error("Failed to create Chronos Warden entity");
        }
    }
    ...
    public static void reset() {
        ...
        tickCounter = 0;
        lastWorldId = null;
        Chronosphere.LOGGER.info("Chronos Warden Spawner reset");
    }
}
