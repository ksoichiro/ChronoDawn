 ...
 *
 * 1.21.9: render() replaced with submit().
 *
...
public class GearProjectileRenderer extends EntityRenderer<GearProjectileEntity, GearProjectileRenderState> {
    ...
    @Override
    public void submit(GearProjectileRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        poseStack.pushPose();
        ...
        // Render as iron ingot (placeholder for gear) using Client Items system
        ...
        // 1.21.9: render() replaced with submit(PoseStack, SubmitNodeCollector, int light, int overlay, int color)
        itemRenderState.submit(poseStack, collector, 0xF000F0, net.minecraft.client.renderer.texture.OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
    ...
    // Texture is obtained directly in submit() method
}
