...
public class PortalStabilizerItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!clickedBlock.is(ModBlocks.CLOCKSTONE_BLOCK.get())) {
            if (player != null) {
                player.sendOverlayMessage(Component.translatable("item.chronodawn.portal_stabilizer.invalid_block"));
            }
        ...
        if (portal == null) {
            if (player != null) {
                player.sendOverlayMessage(Component.translatable("item.chronodawn.portal_stabilizer.no_portal"));
            }
            return InteractionResult.FAIL;
        }
        ...
        if (portal.getCurrentState() == PortalState.STABILIZED) {
            if (player != null) {
                player.sendOverlayMessage(Component.translatable("item.chronodawn.portal_stabilizer.already_stabilized"));
            }
            return InteractionResult.FAIL;
        }
        ...
        if (!portal.getCurrentState().canBeStabilized()) {
            if (player != null) {
                player.sendOverlayMessage(Component.translatable("item.chronodawn.portal_stabilizer.cannot_stabilize"));
            }
            return InteractionResult.FAIL;
        }
