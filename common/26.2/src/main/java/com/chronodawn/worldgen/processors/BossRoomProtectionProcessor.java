 ...
 */
public class BossRoomProtectionProcessor implements StructureProcessor {
    ...
    // replaced by a bare "templateRelativePos" BlockPos (the untransformed original info is no
    // longer passed in per-block; it's only available in finalizeProcessing's originalBlockInfoList).
    // Unused here since this processor never read originalBlockInfo.
    @Nullable
    @Override
    public StructureTemplate.StructureBlockInfo processBlock(
        ...
        // In 1.21.5, getString returns Optional<String>, use getStringOr with default
        String markerType = nbt.getStringOr("MarkerType", "");
        String replaceWith = nbt.getStringOr("ReplaceWith", "");
        ...
        for (Long placementId : bossRoomMinsByPlacement.keySet()) {
            ...
            for (int i = 0; i < pairCount; i++) {
                ...
                ChronoDawn.LOGGER.debug(
                    ...
                    level.dimension().identifier(), minPos, minMarker.placementId, maxPos, maxMarker.placementId, distance, bossRoomArea
                );
        ...
        for (Long placementId : minMarkersByPlacement.keySet()) {
            ...
            for (int i = 0; i < pairCount; i++) {
                ...
                ChronoDawn.LOGGER.debug(
                    ...
                    level.dimension().identifier(), minPos, minMarker.placementId, maxPos, maxMarker.placementId, distance, protectedArea
                );
