...
public class PhantomCatacombsBossRoomPlacer {
    private static final Identifier PHANTOM_CATACOMBS_ID = CompatResourceLocation.create(
        ChronoDawn.MOD_ID,
        "phantom_catacombs"
    );

    private static final Identifier ROOM_7_TEMPLATE = CompatResourceLocation.create(
        ChronoDawn.MOD_ID,
    ...
    private static final Map<Identifier, Set<BlockPos>> processedStructures = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    // ConcurrentHashMap for thread-safe access across multiple dimension ticks
    private static final Map<Identifier, Integer> tickCounters = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    private static class StructureProcessingState {
        ...
        StructureProcessingState(BlockPos origin, ChunkPos chunkPos, Identifier dimId, BoundingBox bbox) {
        ...
        for (var entry : structureManager.getAllStructuresAt(chunkPos.getWorldPosition()).entrySet()) {
            ...
            if (structureLocation != null && structureLocation.equals(PHANTOM_CATACOMBS_ID)) {
                ...
                for (long chunkLong : chunks) {
                    structureChunks.add(ChunkPos.unpack(chunkLong));
                }
            }
        }
    ...
    private static net.minecraft.world.level.levelgen.structure.BoundingBox getStructureBoundingBox(ServerLevel level, ChunkPos chunkPos) {
        ...
        for (var entry : structureManager.getAllStructuresAt(chunkPos.getWorldPosition()).entrySet()) {
            ...
            if (structureLocation != null && structureLocation.equals(PHANTOM_CATACOMBS_ID)) {
                ...
                int validStarts = 0;
                for (long chunkLong : chunks) {
                    ChunkPos structureChunk = ChunkPos.unpack(chunkLong);
    ...
    public static void processStructure(ServerLevel level, ChunkPos chunkPos) {
        ...
        if (structureStart == null || !structureStart.isValid()) {
            ...
            ChunkPos originChunkPos = ChunkPos.containing(structureOrigin);
    ...
    private static void progressCandidateEvaluation(ServerLevel level, StructureProcessingState state) {
        ...
        if (state.currentCandidateIndex >= state.candidates.size()) {
            ...
            if (!collisionFree.isEmpty()) {
                state.selectedCandidate = collisionFree.get(level.getRandom().nextInt(collisionFree.size()));
                ChronoDawn.LOGGER.debug(
                ...
                if (!minimalCollision.isEmpty()) {
                    state.selectedCandidate = minimalCollision.get(level.getRandom().nextInt(minimalCollision.size()));
                    ChronoDawn.LOGGER.warn(
    ...
    private static boolean placeBossRoomIndependently(
        ...
        template.placeInWorld(level, hiddenPos, hiddenPos, settings, level.getRandom(), 2);
