...
public class PulseHogEntity extends Animal {
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(ModItemTags.PULSE_HOG_FOOD);
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return com.chronodawn.registry.ModEntities.PULSE_HOG.get().create(level, EntitySpawnReason.BREEDING);
    }

    // === Sounds (same as vanilla Pig) ===
    // 26.1.2: PIG_AMBIENT/PIG_HURT/PIG_DEATH were removed in favor of the new
    // PigSoundVariant registry system. We always use the "classic" adult variant,
    // matching the old hardcoded vanilla sounds.

    private SoundEvent pigSound(java.util.function.Function<
        ...
        return selector.apply(
            this.registryAccess().lookupOrThrow(Registries.PIG_SOUND_VARIANT)
                .getValueOrThrow(PigSoundVariants.CLASSIC)
                .adultSounds()
        ).value();
    }
    ...
    protected SoundEvent getAmbientSound() {
        return pigSound(net.minecraft.world.entity.animal.pig.PigSoundVariant.PigSoundSet::ambientSound);
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return pigSound(net.minecraft.world.entity.animal.pig.PigSoundVariant.PigSoundSet::hurtSound);
    }

    @Override
    protected SoundEvent getDeathSound() {
        return pigSound(net.minecraft.world.entity.animal.pig.PigSoundVariant.PigSoundSet::deathSound);
    }
