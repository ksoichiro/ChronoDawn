...
public class TickingSheepEntity extends Animal implements Shearable {
    ...
    @Override
    public void shear(ServerLevel serverLevel, SoundSource source, ItemStack shears) {
        serverLevel.playSound(null, this, SoundEvents.SHEEP_SHEAR, source, 1.0F, 1.0F);
        this.setSheared(true);
        int woolCount = 1 + this.random.nextInt(3);
        for (int i = 0; i < woolCount; i++) {
            ItemEntity itemEntity = this.spawnAtLocation(serverLevel, new ItemStack(Items.WOOL.pick(DyeColor.LIGHT_BLUE)));
            if (itemEntity != null) {
    ...
    protected void dropCustomDeathLoot(ServerLevel level, DamageSource damageSource, boolean recentlyHit) {
        ...
        if (!this.isSheared()) {
            this.spawnAtLocation(level, new ItemStack(Items.WOOL.pick(DyeColor.LIGHT_BLUE)));
        }
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return com.chronodawn.registry.ModEntities.TICKING_SHEEP.get().create(level, EntitySpawnReason.BREEDING);
    }
