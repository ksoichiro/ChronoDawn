...
import com.chronodawn.client.renderer.mobs.ChronalLeechRenderState;
 ...
 */
public class ChronalLeechModel extends EntityModel<ChronalLeechRenderState> {
    ...
    @Override
    public void setupAnim(ChronalLeechRenderState state) {
        super.setupAnim(state);

        // Body segments wiggle animation
        float wiggle = Mth.cos(state.ageInTicks * 0.6F) * 0.1F;
        ...
        if (state.attackTime > 0.0F) {
            float strike = Mth.sin(state.attackTime * (float)Math.PI);
