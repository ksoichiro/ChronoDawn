...
import com.chronodawn.client.renderer.mobs.ChronoUrsidRenderState;
 ...
 * Adapted for the 1.21.2+ RenderState pattern.
 *
 * Leg layout (Minecraft's +x = mob's left, -z = mob's front):
 *   leg1 = back  left  (+x, +z)
 *   leg2 = back  right (-x, +z)
 *   leg3 = front left  (+x, -z)
 *   leg4 = front right (-x, -z)
 *
 * Diagonal pairs walk in phase: (leg1 + leg4), (leg2 + leg3).
 * When standing on hind legs (polar-bear behavior), the front legs
 * (leg3, leg4) lift; the head also tilts back slightly.
 */
public class ChronoUrsidModel extends EntityModel<ChronoUrsidRenderState> {
    ...
    @Override
    public void setupAnim(ChronoUrsidRenderState state) {
        super.setupAnim(state);

        float stand = state.standAnimationScale;
        stand = stand * stand;
        ...
        this.head.xRot = state.xRot * ((float) Math.PI / 180F) + stand * (-(float) Math.PI / 4F);
        ...
        this.leg1.xRot = Mth.cos(walkPos * 0.6662F) * 1.4F * walkSpeed * ground - stand * 0.3F;
        this.leg2.xRot = Mth.cos(walkPos * 0.6662F + (float) Math.PI) * 1.4F * walkSpeed * ground - stand * 0.3F;
        this.leg3.xRot = Mth.cos(walkPos * 0.6662F + (float) Math.PI) * 1.4F * walkSpeed * ground - stand * 1.0F;
        this.leg4.xRot = Mth.cos(walkPos * 0.6662F) * 1.4F * walkSpeed * ground - stand * 1.0F;
    }
}
