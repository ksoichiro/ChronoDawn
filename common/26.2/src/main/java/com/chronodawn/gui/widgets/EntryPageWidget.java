...
public class EntryPageWidget extends AbstractWidget {
    ...
    private static class VirtualPage {
        ...
        final Identifier recipe; // For recipe pages
        final Identifier image; // For image pages
        ...
        VirtualPage(Page.PageType type, Identifier image) {
            this.type = type;
            this.lines = null;
            this.recipe = null;
            this.image = image;
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        if (virtualPages.isEmpty()) {
            ...
            graphics.text(font, placeholderText, textX, textY, 0xFF808080, false);
            return;
        }
        ...
        if (pageNumberText != null) {
            ...
            graphics.text(font, pageNumberText, textX, textY, 0xFF3F3F3F, false);
        }
    }
     ...
     */
    private void renderTextVirtualPage(GuiGraphicsExtractor graphics, VirtualPage virtualPage) {
        Font font = Minecraft.getInstance().font;
        ...
        for (String line : virtualPage.lines) {
            graphics.text(font, line, textX, textY, 0xFF000000, false);
            textY += LINE_HEIGHT;
        }
    }
     ...
     */
    private void renderRecipeVirtualPage(GuiGraphicsExtractor graphics, VirtualPage virtualPage) {
        ...
        for (String line : lines) {
            graphics.text(font, line, textX, textY, 0xFF808080, false);
            textY += LINE_HEIGHT;
        }
    }
     ...
     */
    private void renderImageVirtualPage(GuiGraphicsExtractor graphics, VirtualPage virtualPage) {
        Identifier imageLocation = virtualPage.image;
        ...
        try {
            ...
            // Use Matrix3x2fStack (1.21.6: GuiGraphicsExtractor.pose() returns Matrix3x2fStack) to scale the image
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, imageLocation, 0, 0, 0.0f, 0.0f,
            ...
            for (String line : lines) {
                graphics.text(font, line, textX, textY, 0xFFFF0000, false);
                textY += LINE_HEIGHT;
            }
        }
    }
     ...
     */
    private void renderVignetteEffect(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
     ...
     */
    private int[] getImageDimensions(Identifier imageLocation) {
        try {
            ...
            Identifier resourcePath = CompatResourceLocation.create(
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean consumed) {
        ...
        if (previousButton != null && previousButton.mouseClicked(event, consumed)) {
            return true;
        }
        if (nextButton != null && nextButton.mouseClicked(event, consumed)) {
            return true;
        }
