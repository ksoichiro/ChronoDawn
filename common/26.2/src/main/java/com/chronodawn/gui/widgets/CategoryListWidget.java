...
public class CategoryListWidget extends AbstractWidget {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (currentY + TITLE_HEIGHT > getY() && currentY < getY() + height) {
            ...
            graphics.text(Minecraft.getInstance().font, title, titleX, titleY, 0xFF3F3F3F, false);
        }
        ...
        for (Category category : categories) {
            ...
            if (currentY + CATEGORY_HEIGHT > getY() && currentY < getY() + height) {
                ...
                graphics.text(Minecraft.getInstance().font, expandIndicator,
                    getX() + 2, currentY + (CATEGORY_HEIGHT - 8) / 2, 0xFF5F5F5F, false);

                // Render category icon
                ItemStack icon = getIconStack(category);
                graphics.item(icon, getX() + PADDING + 10, currentY + PADDING);
                ...
                graphics.text(Minecraft.getInstance().font, displayTitle, textX, textY, textColor, false);
            ...
            if (isExpanded) {
                for (Entry entry : category.getEntries()) {
                    ...
                    if (currentY + ENTRY_HEIGHT > getY() && currentY < getY() + height) {
                        ...
                        graphics.item(entryIcon, getX() + ENTRY_INDENT + PADDING, currentY + 1);
                        ...
                        graphics.text(Minecraft.getInstance().font, displayText, entryTextX, entryTextY, entryTextColor, false);
    ...
    private ItemStack getIconStack(Category category) {
        Identifier iconId = category.getIcon();
    ...
    private ItemStack getIconStack(Entry entry) {
        Identifier iconId = entry.getIcon();
        var item = BuiltInRegistries.ITEM.get(iconId).map(holder -> holder.value()).orElse(Items.AIR);
        if (item == Items.AIR) {
