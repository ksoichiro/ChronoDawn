...
/**
 * Era C variant (MC 1.20.1) — the oldest supported version. 1.20.1 predates the
 * {@code Holder<MobEffect>} migration, so {@link MobEffects} constants are raw
 * {@link MobEffect} references and {@link MobEffectInstance} constructors take
 * {@code MobEffect} directly (not {@code Holder<MobEffect>}).
 *
 * <p>This copy uses the OLD {@link MobEffects} constant names
 * (MOVEMENT_SLOWDOWN / DIG_SLOWDOWN / MOVEMENT_SPEED) shared with Era B (1.21.1 /
 * 1.21.2 / 1.21.4). The shared copy at {@code common/shared/} uses the newer
 * SLOWNESS / MINING_FATIGUE / SPEED names for Era A (1.21.5+) and is excluded
 * from this module via {@code common/1.20.1/build.gradle}'s {@code java.filter.exclude}.</p>
 *
 * <p>Another Era C difference: {@link ItemStack#hurtAndBreak} takes a Consumer
 * instead of an {@link EquipmentSlot}. See {@link #tryConsumeEcho} for the call
 * site.</p>
 */
public final class ChronoShieldEffectHandler {
    ...
    private static final Set<ResourceLocation> SHORTENED_EFFECTS = Set.of(
        BuiltInRegistries.MOB_EFFECT.getKey(MobEffects.MOVEMENT_SLOWDOWN),
        BuiltInRegistries.MOB_EFFECT.getKey(MobEffects.WEAKNESS),
        BuiltInRegistries.MOB_EFFECT.getKey(MobEffects.DIG_SLOWDOWN)
    );
