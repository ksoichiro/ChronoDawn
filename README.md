...
- **Configurable**: Tune select gameplay values via `config/chronodawn.toml` — Ancient Ruins generation, the four Chrono-specific ore rates and Y ranges, per-boss health/damage multipliers, and the Chrono dimension's ambient Time Distortion strength and scope. See [docs/configuration.md](docs/configuration.md). Modpack creators see [docs/modpack-integration.md](docs/modpack-integration.md).
